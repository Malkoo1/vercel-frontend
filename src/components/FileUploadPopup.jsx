...
import { uploadMultipleFiles } from "../services/api"; // Import your API function (adjust path as needed)
...
const FileUploadPopup = ({ isOpen, onClose }) => {
  ...
  const { folderId, handleFolderSelect, showToast } = useFolders(); // Use FolderContext to get selectedFolder
  const [uploading, setUploading] = useState(false); // State to indicate uploading status

  // const openPopup = () => { // No need for openPopup, controlled by SubHeader now via props
  //     setIsPopupOpen(true);
  // };

  // const closePopup = () => { // Controlled by onClose prop
  ...
  const handleContinueUpload = async () => {
    if (!folderId) {
      showToast("Please select a folder first.", "warning"); // Use CustomToast
      // alert("Please select a folder first."); // Or handle this more gracefully
      return;
    }

    if (uploadedFiles.length === 0) {
      showToast("Please select files to upload.", "warning"); // Use CustomToast
      // alert("Please select files to upload."); // Or handle this more gracefully
      return;
    }
    ...
    try {
      ...
      showToast("Files uploaded successfully!", "success"); // Use CustomToast

      console.log("Files uploaded successfully:", response);
      handleFolderSelect(folderId);
      setUploadedFiles([]); // Clear uploaded files on success
      onClose(); // Close the popup
      // Optionally, you might want to refresh the file list in the FolderContext here
    } catch (error) {
      console.error("Error uploading files:", error);
      showToast("Failed to upload files. Please try again.", "danger"); // Use CustomToast
    } finally {
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-2xl w-full">
            <div className="flex flex-col items-center mb-6">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Start create and manage your project by few clicks!
              </p>
            </div>
